...
const createContact = async (req, res) => {
    ...
    {
        res.status(500).json(response.error || 'There was an error while trying to create the contact.');
    }
};
...
const updateContact = async (req, res) => {
    ...
    if (response.modifiedCount > 0) {
        ...
        res.status(500).json(response.error || 'There was an error while trying to update the contact.');
    }
};
...
const deleteContact = async (req, res) => {
    ...
    if (response.deletedCount > 0) {
        ...
        res.status(500).json(response.error || 'There was an error while trying to delete the contact.');
    }
};
